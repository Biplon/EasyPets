...
public class PetPufferFish extends EntityPufferFish implements PetPlaySound
{
    ...
    public PetPufferFish(Location loc, Player p)
    {
        super(EntityTypes.PUFFERFISH, ((CraftWorld) Objects.requireNonNull(loc.getWorld())).getHandle());
    ...
    protected void mobTick()
    {
        if (goal != null)
        {
            ...
            {
                double d0 = this.goal.locX() + 0.5D - this.locX() + (Math.random() * 2) + 1;
                double d1 = this.goal.locY() + 0.1D - this.locY();
                double d2 = this.goal.locZ() + 0.5D - this.locZ() + (Math.random() * 2) + 1;
